...
export const FILTERS: Record<CatId, FilterDef[]> = {
  ...
  soundcard: [F("brand", "Marca", "enum"), F("channels", "Canales", "enum"), F("snr", "SNR", "range", "dB"),
    F("iface", "Interfaz", "enum"), F("watt", "Consumo", "range", "W"), F("year", "Año", "range")],
  netwired: [F("brand", "Marca", "enum"), F("speed", "Velocidad", "enum"), F("ports", "Puertos", "range"),
    F("iface", "Interfaz", "enum"), F("watt", "Consumo", "range", "W"), F("year", "Año", "range")],
  netwireless: [F("brand", "Marca", "enum"), F("wifi", "Norma Wi-Fi", "enum"), F("bt", "Bluetooth", "has"),
    F("speed", "Clase", "enum"), F("antennas", "Antenas", "range"), F("iface", "Interfaz", "enum"),
    F("watt", "Consumo", "range", "W")],
...
export const KEYSPECS: { [K in CatId]: (p: CatMap[K]) => KeySpecEntry[] } = {
  ...
  soundcard: (p) => [p.channels, `${p.snr} dB SNR`, p.iface],
  netwired: (p) => [p.speed, `${p.ports} puerto${p.ports > 1 ? "s" : ""}`, p.iface],
  netwireless: (p) => [p.wifi, p.speed, p.bt ? `BT ${p.bt}` : "Sin BT", p.iface],
