  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Resumen de la torre" onClick={onClose}
      ...
      <div className="panel fade" onClick={(e) => e.stopPropagation()}
          ...
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(tiles.length, 5)},1fr)`,
            ...
            {tiles.map(([v, l], i) => (
              <div key={i} style={{ padding: "11px 10px", textAlign: "center",
                ...
                <div className="mono" style={{ fontSize: 15, color: "var(--text)", fontWeight: 600 }}>{v}</div>
        ...
        {missing.length > 0 && (
          <div className="mono" style={{ fontSize: 11, color: "var(--warning)", padding: "9px 16px",
        ...
        {fails > 0 && (
          <div className="mono" style={{ fontSize: 11, color: "var(--danger)", padding: "9px 16px",
            borderBottom: "1px solid var(--border)", lineHeight: 1.5 }}>
            ...
            <div key={p._uid} style={{ display: "flex", gap: 10, alignItems: "flex-start",
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 13.5, fontWeight: 600, marginTop: 1 }}>
                  {p.brand} {p.name}
                  {(p.qty || 1) > 1 && <span className="mono" style={{ color: "var(--text-secondary)", fontSize: 11 }}> ×{p.qty}</span>}
                </div>
                <div className="mono" style={{ fontSize: 10, color: "var(--text-secondary)", marginTop: 3 }}>
                  {keyspecsFor(p).join(" · ")}
                </div>
              </div>
              <span className="mono" style={{ fontSize: 12.5, whiteSpace: "nowrap",
                color: p.price ? "var(--text)" : "var(--text-secondary)" }}>
        ...
        <div style={{ borderTop: "1px solid var(--border)", padding: "12px 16px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-end" }}>
            <div className="mono" style={{ fontSize: 10.5, color: "var(--text-secondary)", lineHeight: 1.7 }}>
              Consumo {power.total} W peor caso · {power.gaming} W en juego<br />
              Fuente sugerida <b style={{ color: "var(--text)" }}>{power.rec} W</b> ·{" "}
              <span style={{ color: fails ? "var(--danger)" : warns ? "var(--warning)" : "var(--success)" }}>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div className="mono" style={{ fontSize: 21, color: "var(--accent)", fontWeight: 600, lineHeight: 1.1 }}>
                {eur(totalTower)} {R.cur}
              </div>
              {rest > 0 && <div className="mono" style={{ fontSize: 9.5, color: "var(--text-secondary)", marginTop: 2 }}>
