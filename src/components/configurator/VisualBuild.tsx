...
function PartShape({ part, profile, active, onActivate, onInspect }: { part: VisualPart; profile: VisualHardwareProfile; active: boolean; onActivate: () => void; onInspect: () => void }) {
  ...
  if (part.category === "gpu") {
    ...
    return <g {...common}><path className="visual-solid" d={`M${z.x} ${z.y + 4}h${length - 12}l12 8v${z.h - 12}H${z.x}z`} />{installed && <><circle cx={z.x + 34} cy={z.y + 19} r="12" /><circle cx={z.x + 74} cy={z.y + 19} r="12" /><path d={`M${z.x + 4} ${z.y + 34}h${length - 15}`} />{plugs.map((pins, i) => <g className="visual-gpu-power" key={`${pins}-${i}`}><rect x={z.x + length - 17 - i * 12} y={z.y + 1} width={pins === 16 ? 10 : 8} height="6" rx="1"/><text x={z.x + length - 15 - i * 12} y={z.y + 5}>{pins}</text></g>)}<text className={`visual-gpu-label is-${part.metadata.vendor}`} x={z.x + 103} y={z.y + 24} textAnchor="middle" textLength="45" lengthAdjust="spacingAndGlyphs">{String(part.metadata.family || "GPU")}</text></>}</g>;
  }
