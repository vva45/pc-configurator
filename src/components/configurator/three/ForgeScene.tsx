...
function Interactive({ part, children, onSelect, onHover }: { part: Visual3DPart; children: React.ReactNode; onSelect: (p: Visual3DPart) => void; onHover: (p?: Visual3DPart) => void }) {
  return <group position={part.position} rotation={part.rotation} scale={part.kind === "aio" ? [1,1,1] : part.scale} onClick={(e) => { e.stopPropagation(); onSelect(part); }} onPointerOver={(e) => { e.stopPropagation(); document.body.style.cursor = "pointer"; onHover(part); }} onPointerOut={() => { document.body.style.cursor = ""; onHover(); }}>{children}</group>;
}
...
function Tubes({ part, active }: { part: Visual3DPart; active: boolean }) {
  const target = part.connectionTarget || [-1.2, -1.2, .5];
  const curves = useMemo(() => [-.1, .1].map((offset) => new CatmullRomCurve3([new Vector3(-.72 + offset, -.1, .18), new Vector3(-.9 + offset, -.35, .35), new Vector3(target[0] + offset, target[1] * .55, target[2]), new Vector3(target[0] + offset, target[1], target[2])])), [target]);
...
function ComponentMesh({ part, active }: { part: Visual3DPart; active: boolean }) {
  ...
  if (part.kind === "aio") { const radiatorLength = 1.9 * part.scale[0]; const spacing = radiatorLength * .86 / Math.max(1, part.instances - 1); return <group><mesh scale={[radiatorLength,.52 * part.scale[1],.22 * part.scale[2]]}><boxGeometry /><Material part={part} active={active} /></mesh>{Array.from({length:part.instances},(_,i)=><group key={i} position={[(i-(part.instances-1)/2)*spacing,0,.28]}><Fan part={part} active={active} radius={.28}/></group>)}<Tubes part={part} active={active}/><group position={part.connectionTarget || [0,0,0]}><mesh><cylinderGeometry args={[.25,.25,.16,24]}/><Material part={part} active={active}/></mesh></group></group>; }
